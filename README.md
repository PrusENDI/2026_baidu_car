...
Source edits should happen in this Windows repository first. Use the helper
script to sync only the files that need to run on Orin.

Prerequisite on Windows:

- MSYS2 rsync: `C:\msys64\usr\bin\rsync.exe`
- MSYS2 ssh: `C:\msys64\usr\bin\ssh.exe`

Dry-run explicit files:
...
Plan explicit files without connecting to Orin:
...
- Local root: this project directory
- Remote: `jetson@192.168.0.155:/home/jetson/workspaces/baidu_car_2026_official_run_copy/`
- Excludes: `.git/`, `__pycache__/`, datasets, logs, caches, and large model files

The script checks that the remote target stays under `/home/jetson/workspaces/`.
Use `-DryRun` before a real sync when changing file selections.
