...
### 19.11 新模型替换操作要求（2026-08-18）

详细步骤见 `docs/lane-cnn-training-handoff.md` 的 20.13。核心顺序是：用户先明确选定 epoch，
本地记录三份模型文件 SHA256；上传到 Orin 独立 staging 目录并复核哈希；停推理和车辆主进程；
完整备份旧 `lane_model` 与 `config_car.yml`；通过目录重命名切换模型；新模型同步使用
`kappa_action`，旧 D4/R7 使用 `legacy_error`；完成静态加载、架空轮和低速整圈测试。失败时
停止进程并同时恢复模型目录、配置和对应 profile。当前未授权实际部署，本次仅更新文档。
